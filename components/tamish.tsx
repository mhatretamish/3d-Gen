...
import { ExternalLink, Download, ArrowLeft, Sparkles } from "lucide-react"
  ...
  const ExternalLinks = () => (
    <div className="flex items-center space-x-6">
      <a
        ...
        className="flex items-center text-cyan-400 hover:text-cyan-300 transition-colors tracking-normal text-sm sm:text-base"
      >
    ...
    <div className="relative h-[100dvh] w-full">
      ...
      <div className="absolute inset-0 z-10 pointer-events-none">
        ...
        <div className="absolute top-6 left-6 pointer-events-auto">
          <h1 className="text-3xl text-white font-normal tracking-normal flex items-center">
            <Sparkles className="h-6 w-6 mr-2 text-cyan-400" />
            <span>3D Model Generator</span>
          </h1>
          <p className="text-cyan-400 text-sm mt-1 tracking-normal">Powered by Tamish</p>
        </div>
        ...
        {/* Error message */}
        {error && (
          <div className="absolute top-20 left-1/2 transform -translate-x-1/2 bg-black/80 text-white border border-red-500/50 px-4 py-2 rounded-md tracking-normal">
            {error}
          </div>
        )}
        ...
        {!isLoading && modelUrl && !showPromptContainer && (
          <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex items-center gap-4 pointer-events-auto">
            <Button
              onClick={handleBack}
              className="bg-black/70 hover:bg-black text-white border border-cyan-900/50 rounded-full px-4 py-2 flex items-center gap-2"
            >
            ...
            <Button
              ...
              className="bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 text-white rounded-full px-4 py-2 flex items-center gap-2"
            >
          ...
          <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 w-full max-w-3xl px-4 sm:px-0 pointer-events-auto">
            ...
            {isMobile && (
              <div className="mt-6 flex justify-center pointer-events-auto">
                <div className="bg-black/60 backdrop-blur-md rounded-full px-6 py-3 border border-cyan-900/30">
                  <ExternalLinks />
                </div>
              </div>
            )}
